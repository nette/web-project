...
class EmbedNode extends StatementNode
{
	...
	public static function create(Tag $tag, TemplateParser $parser): \Generator
	{
		...
		$blockTag = new Tag('block', [new Token(Token::End, '', $tag->position)], $tag->position, $tag->end, prefix: $tag->prefix);
		$block = new Block(new StringNode('default'), $layer, $blockTag);
		$defaultBlock = $node->createDefaultBlock($block);
		$parser->pushTag($block->tag);
		[$node->blocks] = yield;
		$parser->popTag();
		...
		if ($loose) {
			...
			$parser->blocks[$layer]['default'] = $block;
	 ...
	 */
	private function createDefaultBlock(Block $block): BlockNode
	{
		$node = $block->tag->node = new BlockNode;
		$node->block = $block;
		$node->modifier = new ModifierNode([], position: $block->tag->position);
		$node->position = $block->tag->position;
		return $node;
	}
